    ...
    <div class="tableContent">
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span @if($administrador->estado == "Activo")class="activeState"@else class="inactiveState" @endif>{{ $administrador->estado}}<span></td>
                    <td>{{ $administrador->telefono}}</td>
                    <td>{{ $administrador->ciudadResidencia}}</td>
                    <td>{{ $administrador->direccion}}</td>
                    <td>{{ $administrador->email}}</td>
                    <td>{{ $administrador->rol}}</td>
                    <td class="actionButton">

                        <form action="{{ url('dashboard/administrador/'.$administrador->idAdministrador.'/edit') }}">
                        ...
                        <form action="{{ url('dashboard/administrador/'.$administrador->idAdministrador) }}" method="POST">
                            {{ method_field('DELETE')}}
                            {{ csrf_field() }}
        ...
        <div class="paginatorButtons">
            ...
            @php
                ...
                $registrosTotales = count($administradoresTotales);
                $administradoresActuales = count($administradores);
                $registros = count($administradoresTotales) / session('paginate');
                ...
                echo "<p class='paginatorText'>Mostrando $administradoresActuales de $registrosTotales registros</p>";
    ...
    <div class="forms">
        <div class="createForm">
            <form action="{{url('/dashboard/administrador')}}" method="POST" enctype="multipart/form-data" class="form">
                ...
                <div class="inputContainer">
                    <input type="email" class="input" value="Administrador" style="color: #b8b8b9" autocomplete="off" readonly>
        ...
        <div class="updateForm">
            ...
            <form @isset($administradoresEdit) action="{{ url('dashboard/administrador/'.$administradoresEdit->idAdministrador) }}" method="POST" @endisset enctype="multipart/form-data" class="form" id="secondForm">
                ...
                <div class="updateButtonContainer">
                    <input type="submit" class="updateButton" value="Actualizar" @isset($administradoresEdit) @else disabled @endisset> <a class="cancelButton" @isset($administradoresEdit) href="/dashboard/administrador" @endisset>Cancelar</a>
                </div>
            </form>
        </div>
    </div>
